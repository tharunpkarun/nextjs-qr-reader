...
const { getDeviceId, getFacingModePattern } = require('nextjs-qr-reader/src/getDeviceId')
const havePropsChanged = require('nextjs-qr-reader/src/havePropsChanged')
const createBlob = require('nextjs-qr-reader/src/createBlob')
...
module.exports = class Reader extends Component {
  ...
  check() {
    ...
    let width = Math.floor(legacyMode ? img.naturalWidth : preview.videoWidth ? preview.videoWidth : 0)
    let height = Math.floor(legacyMode ? img.naturalHeight : preview.videoHeight ? preview.videoHeight : 0)
